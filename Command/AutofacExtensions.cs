...
namespace tpldfexplore.Command
{
    ...
    {
        public static void RegisterCommands(this ContainerBuilder builder, string name)
        {

            if (name == "simple")
            {
                builder.RegisterType<StringReader>().As<IReader<int>>().WithParameter(new NamedParameter("count", AppConfig.Iterations));
                builder.RegisterType<Int2WordsProcessor>().As<IProcessor<int, string>>();
                builder.RegisterType<ConsoleWriter<string>>().As<IWriter<string>>();
